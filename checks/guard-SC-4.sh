#!/usr/bin/env bash
# SC-4 — a lowered baseline without a reason exits 1, and an absent remote ref skips the check
#
...
cases=(
    "lowered baseline without reason exits 1"
    "lowered baseline with reason passes"
    "all-zeroes remote sha skips and returns 0"
)
